...
function MetricBar({
  ...
  detail,
  extra,
}: { label: string; percent: number; detail?: string; extra?: ReactNode }) {
  const pct = Math.min(100, Math.max(0, percent))
  const colorClass = barColorClass(pct)
  return (
    <div className="mobile-metric-bar">
      <div className="mobile-metric-label-row">
        <span className="mobile-metric-label">{label}</span>
        <span className="mobile-metric-value">
          {pct.toFixed(1)}%
          {extra != null && <span style={{ marginLeft: 8, fontSize: 13, fontWeight: 400, color: '#94a3b8' }}>{extra}</span>}
        </span>
      </div>
      <div className="mobile-metric-bar-outer">
        <div className={`mobile-metric-bar-inner ${colorClass}`} style={{ width: `${pct}%` }} />
      </div>
      {detail != null && <div className="mobile-metric-detail">{detail}</div>}
    </div>
  )
}
          ...
          >
            ...
            {m == null ? (
              ...
              <>
                ...
                {m.gpu != null && (
                  <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid rgba(71, 85, 105, 0.3)' }}>
                    <MetricBar
                      label="GPU"
